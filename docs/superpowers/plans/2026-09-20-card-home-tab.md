...
- **拖卡片正文里的链接**（卡片里放一个 markdown 链接）→ 不触发卡片拖拽，链接拖拽保持自己的默认行为；松手后 `Home.md` 顺序不变、卡片不加半透明。
- 从系统里往页面上拖一个文件 → 卡片不吞掉它（不触发任何顺序变化）。
- `Ctrl/Cmd + Z` 之外的重启验证：重启 Obsidian 后顺序保持。
- **跨首页视图拖拽（需要在真实 Obsidian 里确认）**：把首页在分屏里再开一份，或把标签页「移动到新窗口」变成弹出窗口，然后在这两个视图之间拖一张卡片。分屏在同一个 document 里，应当正常；**弹出窗口是另一个 document，自定义 MIME 类型的载荷可能读不到**——若读不到，表现是 `dragover` 不放行、松手没有任何反应（不会改错文件）。这一条必须实测；若弹出窗口确实失效，可以在 `getData(CARD_DRAG_TYPE)` 读空时回落到 `text/plain`（代价是拖到编辑器里会插入一个数字）。
