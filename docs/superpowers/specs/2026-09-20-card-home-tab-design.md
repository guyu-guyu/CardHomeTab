...
| 分区 | 内容 |
| --- | --- |
| 页面 | 仪表盘文件（文本输入，读写前先做库内路径规范化）、卡片标题级别、替换新标签页、启动时打开、网格列数 |
| 品牌区 | logo 类型与取值、缩放、颜色、wordmark 文案、字号、字重 |
| 背景 | 类型、亮/暗两套取值、模糊、压暗 |
| 搜索 | 显示开关、结果数、书签与最近文件开关及数量、仅 Markdown、显示路径 |
| 片段 | 列出内置与用户片段及其路径、「刷新片段列表」按钮 |

仪表盘文件这一项原本计划用文件选择器，实际实现为文本输入 + 路径规范化（`normalizeVaultPath`）。原因是收益与成本不成比例：规范化已经消掉了"填错写法导致 `exists()` 永远为 false、点创建也修不好"这个真正的坑，而文件选择器要另写一个 `AbstractInputSuggest` 子类。若以后要补，参照 Task 13 搜索框里的 `CandidateSuggest`。
