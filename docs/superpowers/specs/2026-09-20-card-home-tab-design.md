...
| 分区 | 内容 |
| --- | --- |
| 页面 | 仪表盘文件（带笔记补全的输入框）、卡片标题级别、替换新标签页、启动时打开、网格列数 |
| 品牌区 | logo 类型与取值、缩放、颜色、wordmark 文案、字号、字重 |
| 背景 | 类型、亮/暗两套取值、模糊、压暗 |
| 搜索 | 显示开关、结果数、书签与最近文件开关及数量、仅 Markdown、显示路径 |
| 片段 | 列出内置与用户片段及其路径、「刷新片段列表」按钮 |

仪表盘文件这一项需要**既**能选已有笔记**又**能填一个还不存在的路径（用户可以先把路径写上，再让首页去创建），所以做成带补全的文本输入，而不是纯下拉或纯文本框。补全不是为了方便，是为了堵一个很难查的死局：`getAbstractFileByPath` 是大小写敏感的精确匹配，用户在 Windows/macOS 上把 `Home.md` 打成 `home.md` 时文件明明在库里，`exists()` 却永远为 false，首页一直显示"文件缺失"，而"创建并打开"也修不好——因为文件其实已经存在了。除了补全，读写前还会走一遍 `normalizeVaultPath`，兜住手改 `data.json` 的情况。
